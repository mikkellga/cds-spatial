...
# result > 100000
```
The error shown to inform you to be carefull about units:

"Fejl i Ops.units(result, 100000) : 
...
```{r forest area}
forests_area_m2 <- st_area(forests)
forests_area_m2
```
```{r forest area hist}
hist(forests_area_m2)
```
```{r forests over 20ha}
forests_area_m2_over_20ha <- forests %>% 
  filter(unclass(st_area(forests)) > 200000)
nrow(forests_area_m2_over_20ha)
forests_area_m2_over_20ha
```
```{r plot geometry and name}
forests_area_m2_over_20ha$navn <- as.factor(forests_area_m2_over_20ha$navn)

ggplot(forests_area_m2_over_20ha) +
  geom_sf(aes(fill = factor(navn))) +
...
Excellent! Computing geographic information for your vector layers can be done with functions like `st_area()` and `st_length()`. As you saw in this exercise, these functions produce a result that can be used in additional calculat1ions but you need to be careful because the result is a `units` object that requires a little additional processing like using `unclass()`.
...
forests_geometry <- st_geometry(forests)
plot(forests_geometry)

plot(forests_geometry, col = "darkgreen")
...
elevation <- raster("../data/DNK_msk_alt.grd") # using the raster library
aarhus <- terra::rast("../data/Aarhus_1m.TIF") # using the terra library
...
- elevation: nrow: 396, ncol: 876, nlyr: 346896.
- aarhus: nrow: 3334, ncol: 3334, nlyr: 5.
...
Because the elevation x and y are in degrees and the aarhus x and y are in meters.

```{r investiation elevation}
elevation
```

```{r investigation aarhus}
aarhus
```
I believe it is due to the difference in resolution difference. 
...
# project elevation to Aarhus crs
elevation_aarhus_crs <- project(elevation, crs(aarhus))

# crop elevation to Aarhus extent 
elevation_aarhus_crs <- crop(elevation_aarhus_crs, ext(aarhus), snap = "out")

# zlim for elevation colors
z <- range(values(elevation_aarhus_crs), na.rm = TRUE)

# plot base map
plotRGB(aarhus, stretch = "lin")

# overlay elevations
plot(elevation_aarhus_crs,
     add = TRUE,
     zlim = z,
     alpha = 0.5)
```
...
aarhus <- raster("../data/Aarhus_1m.TIF")
the_crs <- crs(aarhus, asText = TRUE)
playgrounds <- st_transform(playgrounds, the_crs)

elevation <- raster("../data/DNK_msk_alt.grd")
the_crs <- crs(aarhus) # projectRaster needs crs as object, not as above
elevation <- projectRaster(elevation, crs = the_crs) # projectRaster needs "crs="
...
plotRGB(aarhus, alpha = 0.8)
plot(playgrounds, col = "purple", pch = 19, lwd = 2, add = TRUE)
...
# reading the files
library(sfarrow)
bbr <- st_read_parquet("mystery/a_bbr_1990.parquet")

city_border <- st_read("mystery/a_city_border1988.shp")
districts <- st_read("mystery/districts2023.geojson")
aarhus_mun <- st_read("mystery/aarhus_mun.geojson")
```
```{r}
# verify CRSs
st_crs(bbr) == st_crs(city_border)
st_crs(city_border) == st_crs(districts)
st_crs(districts) == st_crs(aarhus_mun)
```
```{r}
plot(aarhus_mun)
plot(districts)
...
tm_shape(aarhus_mun) +
  ...
  tm_shape(bbr) +
  tm_dots(size = 0.02, col = "purple", alpha = 0.6)
```

```{r}
high_rises <- bbr %>% 
  dplyr::filter(byg054AntalEtager > 6)
...
```{r}
library(tidyverse)
bbr_etager_na_omit <- bbr %>% 
  filter(!is.na(byg054AntalEtager)) %>% 
  arrange()

ggplot(na.rm = TRUE) +
  geom_sf(data = aarhus_mun, fill = NA, linewidth = 0.6) +
  geom_sf(data = bbr_etager_na_omit, aes(color = byg054AntalEtager),
          size = 0.8, alpha = 1) +
  labs(color = "Antal etager") +
  scale_colour_continuous(palette = c("orange", "purple")) +
  ggtitle("Antal Etager på Aarhusianske Bygninger")
  theme_minimal()
