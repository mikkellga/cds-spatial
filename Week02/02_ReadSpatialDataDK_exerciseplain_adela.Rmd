...
forests$area_m2 <- st_area(forests)

#quick histogram
hist(unclass(forests$area_m2))

#quick plot, note that you can use sf functions st_... in a pipeline
forests %>% 
  filter(unclass(area_m2) > 200000) %>% 
  st_geometry() %>% 
  plot()

# alternate plotting of a new object
forests_over20ha <- forests %>% 
  filter(unclass(area_m2) > 200000) 

plot(st_geometry(forests_over20ha))

# plotting with a categorical column
plot(forests_over20ha['navn'])
```
...
plot(forests$geometry, col = "darkgreen");
plot(shelters$geometry, pch =20, col = "red", add =T)
plot(parks$geometry, col = "green", add =T)
plot(playgrounds$geometry, pch =21, col = "purple", add =T) # these do not plot


plot(playgrounds$geometry, pch =21, col = "purple") # they exist, but map ate them ;)
```
...
aarhus1 <- raster("../data/Aarhus_1m.TIF") # reads only layer 1 from RGB composite
plot(aarhus1) # plots only layer 1

aarhus_withraster <- stack("../data/Aarhus_1m.TIF") # reads the full RGB stack
plotRGB(aarhus_withraster) # plots RGB layers on top of each other

# with terra library
aarhus <- rast("../data/Aarhus_1m.TIF") # using the terra library 
...
# defining a missing CRS for lat/long file 
st_crs(playgrounds) <- 4326
st_crs(forests)

# transform playgrounds to UTM 32 (EPSG 25832)
playgrounds_32 <- st_transform(playgrounds, crs = 25832)

st_crs(playgrounds_32) 

# Plot all layers together once CRSs are aligned

plot(forests$geometry, col = "darkgreen");
plot(shelters$geometry, pch =20, col = "red", add =T)
plot(parks$geometry, col = "green", add =T)
plot(playgrounds_32$geometry, pch =20, col = "purple", add =T)
...
elevation_32 <- projectRaster(elevation, crs = 25832)
...
plotRGB(aarhus)
plot(playgrounds_32$geometry, col = "purple", cex = 2, pch = 19, add =T)
