---
title: "Groundwater Pollution in Denmark"
author: "Adela Sobotkova adapted from Johan Horsmans and Emil Jessen"
date: "05/06/2021" revised and updated in April 2025
...
```{r}
library(pacman)
p_load(sf, terra, dplyr, tmap, ggplot2, tidyverse, lubridate, sp, gstat, ggthemes, lmtest)
```

### Loading nitrate data:
Download data from this [sciencedata link](https://sciencedata.dk/themes/deic_theme_oc7/apps/files_sharing/public.php?t=190154e3aa98e3c9dc0cfe726d355963&) You will get a zipped archive and should extract it into a `data/` folder inside Week 12
```{r, results='hide'}
...
sum(lengths(st_intersects(st_as_sf(all_fields),st_as_sf(organic_fields)))>0)
...
plot(conventional_fields$geometry, main = "Conventional fields:")
```

### Taking subset of conventional fields to reduce computational load
```{r}
...
conventional_fields <- conventional_fields %>% 
  st_as_sf() %>% 
  st_join(st_sf(conv_sample), join = st_intersects, left = FALSE)
```
...
plot(conventional_fields$geometry, add = TRUE)
```
...
nitrate_grid <- krige(nitrate_concentration ~ Easting + Northing, nitrate_sp, newdata = spgrid, model = v_model)
